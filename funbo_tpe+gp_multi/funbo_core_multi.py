"""
FunBO core — Multi-Dataset Hybrid GP->TPE Scoring (CIFAR-10, CIFAR-100, Caltech-101).

The paper_score is computed separately for each dataset, then
AVERAGED across datasets. Uses the hybrid GP->TPE evaluator:
  - Phase 1 (T_GP steps): GP with LLM-evolved acquisition function
  - Phase 2 (T - T_GP steps): TPE (Optuna) seeded with GP history
"""
...
T       = 120
T_GP    = 35   # Matches ESTMS paper: 35 GP iters before TPE handoff

PROG_DIR = r"D:\hckthon\our\funbo_fast\funbo_logs_3_cif_cal\v1"
...
def multi_score(AF_fn, T=T, T_GP=T_GP, seeds=SEEDS):
    """
    ...
    for seed in seeds:
        ds_results = run_bo_multi(AF_fn, T=T, T_GP=T_GP, seed=seed)
...
def multi_score_detailed(AF_fn, T=T, T_GP=T_GP, seeds=SEEDS):
    """
...
def main():
    print("=" * 65)
    print("FunBO — Multi-Dataset Hybrid GP->TPE (CIFAR-10, CIFAR-100, Caltech-101)")
    print("=" * 65)
    ...
    # ---- Baselines (multi-dataset, hybrid) ----
    print("Computing baseline scores (multi-dataset, GP->TPE hybrid) ...")
